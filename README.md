...
```bash
cp ~ $MYGIT/AP3D1.homologs.al.fas ~$MYGIT/.
cp AP3D1.homologs.al.fas.treefile ~$MYGIT/.
```
...
```bash
sed 's/*//' ~ $MYGIT/AP3D1/AP3D1.homologs.fas > ~ $MYGIT/AP3D1/AP3D1.homologs.fas
```
...
```bash
rpsblast -query $MYGIT/AP3D1/AP3D1.homologs.fas -db /home/ec2-user/data/Pfam -out ~$MYGIT/AP3D1/AP3D1.rps-blast.out  -outfmt "6 qseqid qlen qstart qend stitle"
```
>``
Now reformat the output so that [EvolView](https://www.evolgenius.info/evolview-v3/) can use it:
```bash
awk 'BEGIN{FS="\t"} {print $1"\t"$2"\t"$3"@"$4"@"$5}' ~ $MYGIT/AP3D1/AP3D1.rps-blast.out | sed 's/,/_/g' | sed 's/ /_/g' | sed 's/__/_/g' | datamash -sW --group=1,2 collapse 3 | sed 's/,/\t/g' | sed 's/@/,/g' > ~ $MYGIT/AP3D1.rps-blast.evol.out
```
...
```bash
awk 'BEGIN{FS="\t"} {gsub(/ |\.|,/, "_", $5)1} {print $1"\t"$2"\t"$3"@"$4"@"$5}' ~$MYGIT/AP3D1/AP3D1.rps-blast.out | awk -F "@" 'BEGIN { OFS=FS }; { print $1,$2, substr($3, 1, 20); }' | datamash -sW --group=1,2 collapse 3 | sed 's/,/\t/g' | sed 's/@/,/g'   > ~AP3D1/AP3D1.rps-blast.evol.out
```
