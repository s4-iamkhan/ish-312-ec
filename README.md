...
The gene code can be found this in a shared google slides file. You can download it directly using the following command:
 ...
 Now, perform a blast search using the query protein:
     ...
     blastp -db ../allprotein.fas -query NP_001248755.1.fa -outfmt 0 -max_hsps 1 -out ARPC.blastp.typical.out
Type in the following command there after: 

    blastp -db ../allprotein.fas -query NP_001248755.1.fa -outfmt "6 sseqid pident length mismatch gapopen evalue bitscore pident stitle"  -max_hsps 1 -out ARPC.blastp.detail.out
...
This will produce a filtered.out file we can use and have a set of putative homologs to the query that is needed for the research.
    ...
    wc -l ARPC.blastp.detail.filtered.out
...
now that proteomes have been downloaded, we need to use seqkit to produce a .fas file of our homologs. 
     ...
     seqkit grep --pattern-file ARPC.blastp.detail.filtered.out ../allprotein.fas > ARPC.homologs.fas
...
```
~/tools/mafft-linux64/mafft.bat --localpair --maxiterate 100  ARPC.homologs.fas   > ARPC.homologs.al.fas 
```
Provide some statistics about the alignment using [t_coffee](https://www.tcoffee.org/Projects/tcoffee/):

     t_coffee -other_pg seq_reformat -in ARPC.homologs.al.fas -output sim

# . Constructing a Phylogenetic Tree for AP3D1 Homologs from Sequence Data

The data for your tree will be the alignment for Hox homologs that you made in lab3:  `ARPC.homologs.al.fas`.

Use IQ-TREE to find the maximum likehood tree estimate, specifying a particular model of substitution (VT+F+R10). 
```bash
iqtree -s ARPC.homologs.al.fas -m VT+F+R10
```
...
### Midpoint rooting
 For now, let's use a type of rooting called midpoint - we'll hope that the root is halfway along the longest branch on the tree. Note: this may not be true, in which case the root will be wrong.
    ...
    gotree reroot midpoint -i ARPC.homologs.al.fas.treefile -o APRC.homologs.al.mid.treefile

Now, we can look at the rooted tree at the command line:
    
	    nw_order -c n ARPC.homologs.al.mid.treefile  | nw_display -
...
``` 
nw_order -c n ARPC.homologs.al.mid.treefile  |   nw_display  -w 1000 -b 'opacity:0' -s  >  ARPC.homologs.al.mid.svg -
```
...
`answers.ARPC.homologs.al.fas.treefile` 


`ls ~ myusername/AP3D1/ARPC.homologs.al.mid.treefile`  

The resulting output should be: `/home/ec2-user myusername/AP3D1/ARPC.homologs.al.mid.treefile`
...
```bash
java -jar ~/tools/Notung-3.0-beta/Notung-3.0-beta.jar -s ../speciesTreeBilateriaCnidaria.tre -g ARPC.homologs.al.mid.treefile --reconcile --speciestag prefix --savepng --events
```
Examine the file ``ARPC.homologs.al.mid.treefile.reconciled.events.txt``. (Hint: use ``less`` with the `-S` command flag, or push this to your remote repository.)

Transfer and then look at the graphic of the reconciled gene tree: ``ARPC.homologs.al.mid.treefile.reconciled.png``
...
```bash
    python2.7 ~/tools/recPhyloXML/python/NOTUNGtoRecPhyloXML.py -g ARPC.homologs.al.mid.treefile.reconciled --include.species
```

The resulting file is called ``ARPC.homologs.al.mid.treefile.reconciled.xml``
...
```bash
java -jar ~/tools/Notung-3.0-beta/Notung-3.0-beta.jar -s ../speciesTreeBilateriaCnidaria.tre -g ARPC.homologs.al.mid.treefile --root --speciestag prefix --savepng --events
```

The output file will be the same as before, but with  "rooting.0" added to the file name. For example: ``ARPC.homologs.al.mid.treefile.rooting.0.events.txt``
...
```bash
cp ~ myusername/ARPC.homologs.al.fas ~myusername/.
cp ARPC.homologs.al.fas.treefile ~myusername/.
```

Then run this command in order to repeat the bootstrap command 

```bash=1
iqtree -s ARPC.homologs.al.fas -nt AUTO -bb 1000 -m VT+F+R10 -t ARPC.homologs.al.fas.treefile -pre ARPC.bs -wbt
```
 This file with the 1000 bootstrapped trees is called `ARPC.bs.ufboot`. (But you will need to run the command for your own gene).
...
```bash
iqtree -t ARPC.bs.ufboot -sup ARPC.homologs.al.fas.treefile 
```
The resulting tree that contains bootstrap values is called ``ARPC.bs.ufboot.suptree``.
...
```bash
gotree reroot midpoint -i ARPC.bs.ufboot.suptree -o ARPC.bs.mid.suptree
```

Finally, print out the tree to a graphics file with the bootstrap support:
```bash
nw_display -s  ARPC.bs.mid.suptree -w 1000 -b 'opacity:0' >  ARPC.bs.mid.suptree.svg
```
...
```bash
nw_order -c n ARPC.bs.mid.suptree | nw_topology - | nw_display -s -w 1000 > ARPC.bs.mid.suptree.Cl.svg -
```
...
We will use `ARPC.homologs.fas`, which were our original, unaligned sequences from lab 3. 
...
```bash
sed 's/*//' ~ myusername/AP3D1/ARPC.homologs.fas > ~ myusername/AP3D1/ARPC.homologs.fas
```
...
```bash
rpsblast -query myusername/AP3D1/ARPC.homologs.fas -db /home/ec2-user/data/Pfam -out ~myusername/AP3D1/ARPC.rps-blast.out  -outfmt "6 qseqid qlen qstart qend stitle"
```
>``
Now reformat the output so that [EvolView](https://www.evolgenius.info/evolview-v3/) can use it:
```bash
awk 'BEGIN{FS="\t"} {print $1"\t"$2"\t"$3"@"$4"@"$5}' ~ myusername/AP3D1/ARPC.rps-blast.out | sed 's/,/_/g' | sed 's/ /_/g' | sed 's/__/_/g' | datamash -sW --group=1,2 collapse 3 | sed 's/,/\t/g' | sed 's/@/,/g' > ~ myusername/AP3D1.rps-blast.evol.out
```
...
```
awk 'BEGIN{FS="\t"} {gsub(/ |\.|,/, "_", $5)1} {print $1"\t"$2"\t"$3"@"$4"@"$5}' ~myusername/AP3D1/ARPC.rps-blast.out | awk -F "@" 'BEGIN { OFS=FS }; { print $1,$2, substr($3, 1, 20); }' | datamash -sW --group=1,2 collapse 3 | sed 's/,/\t/g' | sed 's/@/,/g'   > ~AP3D1/ARPC.rps-blast.evol.out
```
